...
- (void)drawRect:(CGRect)frame {
	...
	for (NSInteger i = 0; i < count; i++) {
		...
		if ([item isKindOfClass:[NSString class]]) {
			...
			CGSize textSize = [string sizeWithFont:_font constrainedToSize:CGSizeMake(segmentWidth, size.height) lineBreakMode:NSLineBreakByTruncatingTail];
			...
			if (enabled) {
				...
				[string drawInRect:CGRectAddPoint(textRect, CGPointMake(_textShadowOffset.width, _textShadowOffset.height)) withFont:_font lineBreakMode:NSLineBreakByTruncatingTail alignment:NSTextAlignmentCenter];
			...
			[string drawInRect:textRect withFont:_font lineBreakMode:NSLineBreakByTruncatingTail alignment:NSTextAlignmentCenter];
		}
