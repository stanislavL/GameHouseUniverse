...
- (void)showOverlay:(UIImage *)_image{
	
	UIImageView *qrOverlayImageView = [[[UIImageView alloc] initWithImage:_image] autorelease];
...
- (void)initSignalAtPosition:(CGPoint)_point{
	
	self.signalImage = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"cell_indicator_red.png"]] autorelease];
	self.signalImage.frame = CGRectMake(_point.x, _point.y, self.signalImage.frame.size.width, self.signalImage.frame.size.height);
	self.signalImage.hidden = YES;
	
	[self addSubview:self.signalImage];
}

- (void)showSignal:(BOOL)_show color:(NSString*)_color{
	
	if(!_show){
		self.backgroundColor = [UIColor clearColor];
	}
	else if([_color isEqualToString:@"green"]){
		self.signalImage.image = [UIImage imageNamed:@"cell_indicator_green.png"];
		self.backgroundColor = [UIColor colorWithRed:0.0 green:1.0 blue:0.0 alpha:0.25];//[UIColor colorWithWhite:1.0 alpha:0.25]
	}
	else{
		self.signalImage.image = [UIImage imageNamed:@"cell_indicator_red.png"];
		self.backgroundColor = [UIColor colorWithRed:1.0 green:0.0 blue:0.0 alpha:0.25];
	}
	
	self.signalImage.hidden = !_show;
}

- (void)showCancelButton{
	
	UIImage* image = [UIImage imageNamed:@"navbar_button_abbrechen.png"];
	UIImage* imagePressed = [UIImage imageNamed:@"navbar_button_abbrechen_pressed.png"];
	UIButton* button = [UIButton buttonWithType:UIButtonTypeCustom];
	
	button.frame = CGRectMake(5, 5, image.size.width, image.size.height);
